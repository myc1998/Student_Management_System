...
<%@ page import="com.myc.tools.*"  import ="java.io.PrintWriter"%>
...
<body>
	...
	try{
		...
		System.out.printf("------2");
		if(rs.next()){
			if(rs.getString("stu_password").equals(password)&&code.equals(randStr)){  //判断从上一页面收到的密码值是否与数据库中的password相等
				System.out.printf("------3");
				student = new Student();
				...
				rs = ps.executeQuery();
				if(rs.next()){
				if(rs.getString("admian_password").equals(password)&&code.equals(randStr)){
				Admain=new admian();
				...
				rs = ps.executeQuery();
				if(rs.next()){
				if(rs.getString("teacher_password").equals(password)&&code.equals(randStr)){
				Teacher=new teacher();
	...
	if(isLogin){
		...
		request.setAttribute("MSG", "用户名或者密码或者验证码错误！");
